...
namespace HorseApp2.Versions.v1_1.Controllers
{
    ...
    public class GeographyRequestController : ApiController
    {
        ...
        [HttpPost]
        [Route("InsertMissingLocationData")]
        public async Task<IHttpActionResult> InsertMissingLocationData(LocationDatabaseModel locationData)
        {
            var requiredParametersMessage =
                "Required parameters are: 'countryCode', 'postalCode', 'latitude', and 'longitude'.";
            if (string.IsNullOrEmpty(locationData.CountryCode))
            {
            ...
            if (float.IsNaN(locationData.Latitude))
            {
                return BadRequest($"Parameter 'latitude' is missing. {requiredParametersMessage}");
            }
            if (float.IsNaN(locationData.Longitude))
            {
                return BadRequest($"Parameter 'longitude' is missing. {requiredParametersMessage}");
            }
            ...
            try
            {
                using (var context = new HorseDatabaseEntities())
                {
                    ...
                    SqlCommand cmd = new SqlCommand("usp_v1_1_InsertMissingLocationData");
